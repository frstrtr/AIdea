...
MODES_TEXT = (
    "Modes — pick exactly one per call. Output is always tied to your topic.\n"
    "\n"
    "/idea — default\n"
    "  n_ideas at the current entropy. Feasibility required. Title, pitch, "
    "mechanism, first step, risks.\n"
    "\n"
    "/einstein — four generative mechanisms\n"
    "  1. Adjacent Possible — name a capability that unlocked in the last "
    "1-3 years, walk through that door\n"
    "  2. Exaptation — transplant a mechanism from a far-distant field "
    "(Gutenberg / wine press)\n"
    "  3. Slow Hunch — articulate a latent tension and propose its "
    "resolution\n"
    "  4. Productive Error — invert one load-bearing assumption the field "
    "treats as fixed\n"
    "  Combine with /set refine=true to rank mechanisms and harden the "
    "winning route.\n"
    "\n"
    "/lsd — Prior Dissolution (predictive processing / REBUS)\n"
    "  Suspend the field's silent interpretive frame and re-categorize "
    "the same situation. Different from inverting one belief — this "
    "dissolves the whole category-membership.\n"
    "\n"
    "/futures — temporal projection\n"
    "  Four passes at +1y / +3y / +10y / +30y horizons. Each names three "
    "concrete shifts at that horizon, identifies what is OBVIOUS from "
    "there but invisible from today, then translates to a v0.1 you ship "
    "this year.\n"
    "\n"
    "/dream — dreaming mode (no feasibility floor)\n"
    "  Prediction-error signal offline. Generative model runs free. "
    "Output: vivid dream image (may violate physics / economics / law) "
    "plus an explicit 'what survives waking' line naming the salvageable "
    "fragment.\n"
    "\n"
    "/lucid <prior> | <topic>\n"
    "  Lucid dream — dream with a directional prior YOU inject. The "
    "hallucination biases toward your belief; one reality check fires "
    "before waking. More salvage than pure dream because the prior "
    "anchors the hallucination. Separator '|' or '::' both work.\n"
    "  Example: /lucid the team must stay solo-founder-sized | growing my consultancy\n"
    "\n"
    "Flags (set via /set):\n"
    "  refine=true       score every idea, pick the winner, refine it at "
    "low entropy\n"
    "  evolve_deck=true  after refine, sharpen the winning cards back into "
    "the deck cache\n"
    "  theme_entropy=X   0..1 — how distant the auto-generated donor "
    "domains should be (0=in-field; 1=wildly distant). Defaults to your "
    "entropy spread.\n"
    "  themes=A,B,C      override the LLM-picked theme list with your own "
    "comma-separated donor domains\n"
)
